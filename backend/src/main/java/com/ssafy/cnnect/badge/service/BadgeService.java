...
public class BadgeService {
    ...
    public List<Badge> getNewBadge(Long categoryId, Long newSentence){
        ...
        System.out.println("videoList : " + videoList.size() + " categoryId : " + categoryId);
        ...
        for(Badge b : categoryBadge){
            boolean isBadgeInCategory = userBadge.stream()
                    .anyMatch(userDto -> categoryBadge.stream()
                            .anyMatch(badge -> userDto.getBadgeId() == badge.getBadgeId()));
//            if(!isBadgeInCategory && b.getBadgeCount()){
//                // 3. 방금 학습으로 인해 그 뱃지를 얻을 수 있게 되었는지 or 아닌지 판별
//                newBadges
//            }
        }
