<template>
  <form class="max-w-md mx-auto mt-10">
    <div class="flex items-center mb-5">
      ...
      <div class="w-3/4 gird grid-cols-2 gap-6">
        <input
          type="email"
          id="email"
          ...
          required
        />
        <button type="button" @click="emailDuplCheck()" class="text-red-700  hover:text-white border border-red-700 hover:bg-red-800 focus:ring-2 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-600 dark:focus:ring-red-900">중복확인</button>
      </div>
    </div>
    <div class="flex items-center mb-5">
      <div class="w-1/4">
      ...
      <div class="w-3/4 gird grid-cols-2 gap-6">
        <input
          ...
          required
        />
        <button type="button" @click="codeSend()" class="text-red-700  hover:text-white border border-red-700 hover:bg-red-800 focus:ring-2 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-600 dark:focus:ring-red-900">인증전송</button>
      </div>
    </div>
    <div class="flex items-center mb-5">
      <div class="w-1/4">
...
const nextStep = (input) => {
    ...
    registUser(formData.value, ({data}) => {
      console.log(data)
      setUserId(data.data);
    },
    (error) => {
...
const codeSend = () => {
  emailSend(formData.value.userEmail, ({data}) => {
    console.log(data);
  }, (error) => {
    console.log(error)
  })
